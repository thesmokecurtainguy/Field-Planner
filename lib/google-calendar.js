...
async function upsertGoogleEvent(calendar, calId, ev, isoDate) {
  const body = toGoogleEventBody(ev, isoDate);
  const existingId = ev.googleEventId ? String(ev.googleEventId) : '';
  ...
  if (existingId) {
    try {
      ...
      // Stale id — create a fresh event below.
    }
  }
  ...
  return { googleEventId: created.data.id, action: existingId ? 'recreated' : 'created' };
}
...
async function syncPayloadToGoogle(payload) {
  ...
  const previouslySynced = readSyncedIds(payload);
  ...
  for (const group of dayGroups) {
    for (const ev of group.syncable) {
      if (ev.googleEventId) currentlyPresent.add(String(ev.googleEventId));
    }
  }

  for (const id of previouslySynced) {
    if (currentlyPresent.has(id)) continue;
    try {
      await deleteGoogleEvent(calendar, calId, id);
      stats.deleted += 1;
    } catch (err) {
      console.error('[google-calendar] delete failed', id, err.message);
  ...
  for (const group of dayGroups) {
    for (const ev of group.syncable) {
      try {
        const result = await upsertGoogleEvent(calendar, calId, ev, group.isoDate);
