...
// ─── Read-only overlay (Google → UI; never mutates payload) ─────────────────

const OVERLAY_CACHE_TTL_MS = 60_000;
/** @type {{ key: string, fetchedAt: number, items: object[] } | null} */
let overlayListCache = null;

/**
 * All Google event IDs Field Planner already owns — from event.googleEventId
 * fields and the __google_synced_ids__ registry. Overlay must exclude these.
 */
function collectKnownGoogleEventIds(payload) {
  const ids = readSyncedIds(payload);
  for (const group of collectDayEvents(payload)) {
    for (const ev of group.allEvents) {
      if (ev && ev.googleEventId) ids.add(String(ev.googleEventId));
    }
  }
  for (const ev of parseJsonArray(payload && payload[MULTIDAY_KEY])) {
    if (ev && ev.googleEventId) ids.add(String(ev.googleEventId));
  }
  return ids;
}

function isIsoDateParam(value) {
  return /^\d{4}-\d{2}-\d{2}$/.test(String(value || ''));
}

/** Format a Date as HH:MM in America/New_York. */
function formatEtHm(date) {
...
/** Calendar date (YYYY-MM-DD) for a Date instant in America/New_York. */
function etIsoDate(date) {
...
/**
 * Map a Google Calendar event → overlay DTO with Field Planner day keys.
 * All-day end.date is exclusive; timed events use America/New_York wall time.
 * Returns null if the event has no usable start.
 */
function googleEventToOverlay(g) {
  if (!g || !g.id || !g.start) return null;
  ...
  if (allDay) {
    startIso = String(g.start.date).slice(0, 10);
    const endExclusive = g.end && g.end.date
      ? String(g.end.date).slice(0, 10)
      : nextIsoDate(startIso);
    // Google all-day end is exclusive — last visible day is the day before.
    endIsoInclusive = startIso < endExclusive
      ? prevIsoDate(endExclusive)
      : startIso;
    if (endIsoInclusive < startIso) endIsoInclusive = startIso;
  } else if (g.start.dateTime) {
    const startDt = new Date(g.start.dateTime);
    if (Number.isNaN(startDt.getTime())) return null;
    startIso = etIsoDate(startDt);
    startTime = formatEtHm(startDt);
    if (g.end && g.end.dateTime) {
      const endDt = new Date(g.end.dateTime);
      if (!Number.isNaN(endDt.getTime())) {
        endIsoInclusive = etIsoDate(endDt);
        endTime = formatEtHm(endDt);
        // Exclusive-ish midnight end (e.g. ends 00:00 next day) → previous calendar day.
        if (endTime === '00:00' && endIsoInclusive > startIso) {
          endIsoInclusive = prevIsoDate(endIsoInclusive);
          endTime = '23:59';
        }
      } else {
        endIsoInclusive = startIso;
      }
    } else {
      endIsoInclusive = startIso;
    }
  ...
  let endDayKey;
  try {
    startDayKey = isoDateToDayKey(startIso);
    endDayKey = isoDateToDayKey(endIsoInclusive);
  } catch (_) {
    return null;
  }
  ...
  return {
    ...
    source: 'google',
    readOnly: true,
    title: String(g.summary || '(untitled)').trim() || '(untitled)',
    description: g.description ? String(g.description) : '',
    location: g.location ? String(g.location) : '',
    allDay,
    startDayKey,
    endDayKey,
    startTime,
    ...
    // Raw Google bounds for debugging / future UI needs — not written to payload.
    googleStart: g.start.date || g.start.dateTime || null,
    googleEnd: (g.end && (g.end.date || g.end.dateTime)) || null,
  };
}

/**
 * List all events on the dedicated calendar in [timeMin, timeMax).
 * Cached briefly so month/week view remounts don't hammer the API.
 */
async function listCalendarEventsInRange(calendar, calId, timeMin, timeMax) {
  const cacheKey = `${calId}|${timeMin}|${timeMax}`;
  const now = Date.now();
  if (
    overlayListCache &&
    overlayListCache.key === cacheKey &&
    now - overlayListCache.fetchedAt < OVERLAY_CACHE_TTL_MS
  ) {
    return { items: overlayListCache.items, cached: true };
  }
  ...
  overlayListCache = { key: cacheKey, fetchedAt: now, items };
  return { items, cached: false };
}

/**
 * Fetch Google-origin events for overlay display.
 * Read-only: does not mutate payload and never writes to app_data.
 *
 * @param {object} payload current household payload (for known-ID filtering only)
 * @param {{ start: string, end: string }} range inclusive ISO dates YYYY-MM-DD
 */
async function fetchOverlayEvents(payload, range) {
  const start = String(range && range.start || '').trim();
  const end = String(range && range.end || '').trim();
  if (!isIsoDateParam(start) || !isIsoDateParam(end)) {
    throw Object.assign(new Error('start and end must be YYYY-MM-DD'), { code: 'BAD_RANGE' });
  }
  if (start > end) {
    throw Object.assign(new Error('start must be on or before end'), { code: 'BAD_RANGE' });
  }

  const calendar = getCalendarClient();
  const calId = calendarId();
  // Inclusive UI range → Google timeMax exclusive (start of day after end).
  const timeMin = `${start}T00:00:00Z`;
  const timeMax = `${nextIsoDate(end)}T00:00:00Z`;

  const { items, cached } = await listCalendarEventsInRange(calendar, calId, timeMin, timeMax);
  const knownIds = collectKnownGoogleEventIds(payload || {});

  const events = [];
  for (const g of items) {
    if (!g || !g.id) continue;
    if (knownIds.has(String(g.id))) continue; // Field Planner already owns this event
    const overlay = googleEventToOverlay(g);
    if (overlay) events.push(overlay);
  }
  ...
  return {
    ...
    meta: {
      ...
      excluded: items.length - events.length,
...
module.exports = {
  ...
  collectKnownGoogleEventIds,
  googleEventToOverlay,
  fetchOverlayEvents,
  syncPayloadToGoogle,
};
